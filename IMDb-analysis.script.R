...
##imdb <- read.csv("Source_files/ratings.csv") - OLD
...
#Insert dates as dates and find difference between rated and release date
imdb.movies$Date.Rated <- as.Date(imdb.movies$Date.Rated, origin = "1900-01-01")
imdb.movies$Release.Date <- as.numeric(imdb.movies$Release.Date)
imdb.movies$Release.Date <- as.Date(imdb.movies$Release.Date, origin = "1900-01-01")
imdb.movies$Dif.rate.release.date <- imdb.movies$Date.Rated - imdb.movies$Release.Date

summary(imdb.movies$Dif.rate.release.date)
mean(imdb.movies$Dif.rate.release.date)
max(imdb.movies$Dif.rate.release.date)
min(imdb.movies$Dif.rate.release.date)

plot(imdb.movies$Dif.rate.release.date, ordered(imdb.movies$Num.Votes))

summary(imdb.movies$Year)
unique(imdb.movies$Year)
...
#plot(imdb$Year, ordered(imdb$Your.Rating))
